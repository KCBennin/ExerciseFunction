...
(define descriptions '((1 "Begin your quest")
                       (2 "You are in the highlands")
                       (3 "You are in the lowlands")
                       (4 "You are in the eastlands")
                       (5 "You are in the westlands")
                       (6 "There you will find her")
                       (7 "surprise")
                       (8 "You are finished")))

(define look '(((directions) look) ((look) look) ((examine room) look)))
(define quit '(((exit game) quit) ((quit game) quit) ((exit) quit) ((quit) quit)))
(define take '(((directions) take) ((take) take) ((examine room) take)))
(define actions `(,@look ,@take ,@quit))



(define decisiontable `((1 ((north) 2) ((north end) 8) ,@actions)
                        (2 ((south) 3) ,@actions)
                        (3 ((east) 4)  ,@actions)
                        (4 ((west) 5) ,@actions)
                        (5 ((western) 6) ,@actions)
